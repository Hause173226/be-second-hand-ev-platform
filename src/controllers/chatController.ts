...
export const getOrCreateChat = async (
  ...
  try {
    ...
    const buyerId = (req as any).user?._id || (req as any).user?.id;
    ...
    let chat = await Chat.findOne({
      ...
      "fullName phone email avatar make model year"
    );
...
export const sendMessage = async (
  ...
  try {
    ...
    try {
      ...
      wsService.sendEnhancedMessageNotification(
        chatId,
        senderId,
        message.content,
        senderInfo
...
export const sendMessageWithPastedImage = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    ...
    if (!imageData.startsWith("data:image/")) {
      res
        .status(400)
        .json({ error: "Dữ liệu hình ảnh phải ở định dạng base64 hợp lệ" });
      return;
    }
    ...
    if (imageData.length > 20 * 1024 * 1024) {
      ...
      res
        .status(400)
        .json({ error: "Hình ảnh quá lớn. Vui lòng chọn hình ảnh nhỏ hơn." });
      return;
    }
    ...
    try {
      ...
      wsService.sendFileUploadNotification(
        chatId,
        messageDoc._id.toString(),
        messageDoc.metadata?.files || [],
        senderInfo
      );
    ...
    if (error instanceof Error) {
      if (error.message.includes("Invalid base64 image format")) {
        res
          ...
          .json({
            error:
              "Định dạng hình ảnh không được hỗ trợ. Chỉ hỗ trợ PNG, JPEG, JPG, GIF và WebP.",
          });
      } else if (error.message.includes("Image size exceeds")) {
        res
          .status(400)
          .json({ error: "Hình ảnh quá lớn. Kích thước tối đa là 10MB." });
      } else if (error.message.includes("Image size too small")) {
        res
          .status(400)
          .json({ error: "Hình ảnh quá nhỏ. Vui lòng chọn hình ảnh khác." });
      } else {
        res.status(500).json({ error: "Lỗi máy chủ nội bộ" });
      }
    } else {
...
export const sendMessageWithFiles = async (
  ...
  try {
    ...
    if (req.files && Array.isArray(req.files) && req.files.length > 0) {
      ...
      const uploadedFiles = await FileUploadService.processUploadedFiles(
      ...
      const detectedMessageType = FileUploadService.getFileType(
        firstFile.mimetype
      );
    ...
    console.log(
      "Saved message:",
      JSON.stringify(messageDoc.toObject(), null, 2)
    );
...
export const getOnlineUsersInChat = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
