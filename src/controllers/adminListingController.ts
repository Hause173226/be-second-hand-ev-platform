...
export const pendingQueue: RequestHandler = async (_req, res, next) => {
  try {
    const list = await Listing.find({ status: "PendingReview" }).sort({ createdAt: 1 });
    res.json(list);
  } catch (err) { next(err); }
};
...
export const approveListing: RequestHandler = async (req, res, next) => {
  try {
    ...
    if (listing.status !== "PendingReview") { res.status(409).json({ message: "Chỉ duyệt khi PendingReview" }); return; }
...
export const rejectListing: RequestHandler = async (req, res, next) => {
  try {
    ...
    const listing = await Listing.findById(id);
    if (!listing) { res.status(404).json({ message: "Listing không tồn tại" }); return; }
    if (listing.status !== "PendingReview") { res.status(409).json({ message: "Chỉ từ chối khi PendingReview" }); return; }
