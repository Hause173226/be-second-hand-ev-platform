...
async function migrateData() {
  try {
    ...
    // Import models
    const SystemWallet = mongoose.model(
      "SystemWallet",
      new mongoose.Schema({}, { strict: false })
    );
    const SystemWalletTransaction = mongoose.model(
    ...
    const EscrowAccount = mongoose.model(
      "EscrowAccount",
      new mongoose.Schema({}, { strict: false })
    );
    ...
    if (existingTxCount > 0) {
      console.log(
        "⚠️  Đã có dữ liệu trong SystemWalletTransaction. Bạn có muốn tiếp tục? (có thể tạo duplicate)"
      );
      console.log(
        "   → Nếu muốn migrate lại, hãy xóa collection SystemWalletTransaction trước"
      );
      return;
    }
    ...
    console.log(
      `   - Tìm thấy ${completedAppointments.length} appointments đã hoàn thành`
    );
    ...
    for (const appointment of completedAppointments) {
      if (!appointment.depositRequestId) {
        console.log(
          `   ⚠️  Skip appointment ${appointment._id}: không có depositRequestId`
        );
        skippedCount++;
        continue;
      }
      ...
      if (!depositRequest) {
        console.log(
          `   ⚠️  Skip appointment ${appointment._id}: không tìm thấy depositRequest ${appointment.depositRequestId}`
        );
        skippedCount++;
        continue;
      }
      ...
      if (escrow && escrow.status !== "RELEASED") {
        console.log(
          `   ⚠️  Appointment ${appointment._id}: escrow status = ${escrow.status} (không phải RELEASED), nhưng vẫn migrate vì appointment đã COMPLETED`
        );
      }
      ...
      if (existingTx) {
        console.log(
          `   ⚠️  Skip appointment ${appointment._id}: đã có transaction rồi`
        );
        skippedCount++;
        continue;
      }
      ...
      completedCount++;
      console.log(
        `   ✅ Created transaction for appointment ${
          appointment._id
        } - Amount: ${depositRequest.depositAmount.toLocaleString("vi-VN")} VND`
      );
    }
    ...
    if (skippedCount > 0) {
      console.log(
        `   ⚠️  Đã skip ${skippedCount} appointments (thiếu dữ liệu hoặc đã có transaction)`
      );
    }
    ...
    console.log(
      `   - Tìm thấy ${cancelledAppointments.length} appointments đã hủy`
    );
    ...
    for (const appointment of cancelledAppointments) {
      ...
      if (escrow && escrow.status !== "REFUNDED") {
        console.log(
          `   ⚠️  Appointment ${appointment._id}: escrow status = ${escrow.status} (không phải REFUNDED), nhưng vẫn migrate vì appointment đã CANCELLED`
        );
      }
      ...
      console.log(
        `   ✅ Created transaction for appointment ${
          appointment._id
        } - Fee: ${feeAmount.toLocaleString("vi-VN")} VND`
      );
    }

    if (cancelledSkippedCount > 0) {
      console.log(
        `   ⚠️  Đã skip ${cancelledSkippedCount} appointments (thiếu dữ liệu hoặc đã có transaction)`
      );
    }
    ...
    const totalCompleted = await SystemWalletTransaction.countDocuments({
      type: "COMPLETED",
    });
    const totalCancelled = await SystemWalletTransaction.countDocuments({
      type: "CANCELLED",
    });
    ...
    console.log(
      `   - Tổng số tiền: ${
        totalAmount[0]?.total?.toLocaleString("vi-VN") || 0
      } VND`
    );

    console.log("\n✅ Migration hoàn tất!");
    console.log(
      "   → Bây giờ bạn có thể test API /api/system-wallet/transactions"
    );
